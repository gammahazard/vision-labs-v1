...
- [x] 17 tool-calling functions: query events/events by date/event patterns, query faces/unknowns, live scene, capture snapshot (with weather + scene description), get weather, browse vehicles, query zones, query notification history, feedback stats, review feedback, retrain rules, send Telegram (text/snapshot/clip), schedule reminders, system status
