...
> **Status:** Phases 0–6 complete. Camera live at 15 FPS, detection + tracking + face re-ID + zones + alerts + Telegram notifications + identity events all working.
> **Date:** Feb 20, 2026
...
9. **Phase 6.5 — Self-learning feedback loop.** 📋 Review queue, user feedback, adaptive alert tuning.
10. **Phase 7 — Give it a brain.** 📋 LLM narration, summaries, chat, anomaly reasoning.
...
| **Vehicle detection / speed estimation** | Scope creep — add after core person pipeline works |
| **License plate reader (OCR)** | Scope creep — cool but not core security |
| **Age/gender estimation** | Scope creep — can add as a Redis worker anytime later |
| **Car make/model via LLM** | Requires vehicle pipeline + LLM — both deferred |
...
- [ ] Telegram notifications include inline feedback buttons
- [ ] Feedback stored in SQLite with event features
- [ ] Dashboard shows review queue with batch approve/reject
- [ ] Alert scoring engine uses feedback to filter noise
- [ ] System shows autonomy level and accuracy on dashboard

---

## Phase 7: LLM Brain — Mistral 7B via Ollama
...
| Component | Runs On | What It Does |
|-----------|---------|--------------|
| LLM service | PC (GPU) | Mistral 7B via Ollama — event narration, summaries, chat |
...
- [ ] LLM narrates detection events in natural language
- [ ] Daily summary auto-generates
- [ ] Chat box on dashboard answers questions about events
- [ ] Review queue shows LLM pre-labels
...
| **LLM (Phase 7)** | Ollama + Mistral 7B | Local, private, on-demand |
...
graph TD
    ...
    style P6_5 fill:#3c1f5c,color:#fff
    style P7 fill:#3c1f5c,color:#fff
...
| **Phase 6.5** | Self-learning feedback loop | 📋 Next |
| **Phase 7** | LLM brain (Ollama + Mistral) | 📋 Future |
