...
> **Status:** Phases 0–12 complete. Camera live at 15 FPS, detection + tracking + face re-ID + zones + alerts + Telegram notifications + vehicle detection + dashboard auth + self-learning feedback + AI assistant + Telegram Access Manager + DVR + QNAP NAS + image generation (SDXL + LoRA + sweeps) + vision analysis (MiniCPM-V) + video production (AnimateDiff + TTS) all working.
> **Date:** Feb 25, 2026
...
11. **Phase 7 — Give it a brain.** ✅ Complete. Ollama + Qwen 3 14B, onboarding wizard, chat UI, 22 tools (events, faces, unknowns, weather, live scene, capture snapshot, capture video clip in chat, patterns, vehicles, zones, notifications, feedback, retrain, Telegram, reminders, system status, activity heatmap, record verdict, show faces, analyze image).
12. **Phase 8 — Extended bot commands.** ✅ Complete. 8 new Telegram commands (/zones, /rules, /night, /faces, /timelapse, /events with thumbnails, /analyze, /ask) + 3 new AI tools.
13. **Phase 9 — QNAP NAS.** ✅ Complete. DVR recording (ffmpeg remux), event journal, Telegram audit trail, 28-day rolling retention.
14. **Phase 10 — Image generation.** ✅ Complete. ComfyUI + SDXL, txt2img/img2img, LoRA support, batch generation, gallery with lightbox, parameter sweeps, prompt history, creative chat (Dolphin model).
15. **Phase 11 — Vision analysis.** ✅ Complete. MiniCPM-V 2.6 multimodal model, describe camera snapshots, analyze uploaded images/videos.
16. **Phase 12 — Video production.** ✅ Complete. AnimateDiff pipeline, AI script generation (Ollama), per-scene animation, TTS narration (Piper), character management with IPAdapter, ffmpeg assembly.
...
- **Continuous recording** → DVR recorder service (`services/recorder/`) writes 1-hour MP4 segments to QNAP NAS via Docker CIFS volume. Zero-transcode (`ffmpeg -c copy`). 28-day rolling retention. ✅ Complete
- **Event clips** → Archive worker saves 10s clips around each detection event (📋 planned)
- **Event journal** → All events logged as JSONL to QNAP NAS (`/data/events/YYYY-MM-DD.jsonl`). ✅ Complete
- **Telegram audit trail** → Per-user command logs + media copies on QNAP NAS. ✅ Complete
...
Camera → Ingester → Redis frames → vehicle-detector (consumer group: vehicle_detectors)
    ...
    → IoU-matched TrackedVehicle objects (0.2 threshold, center history, stationary detection)
    → stores JPEG snapshot in Redis (24h TTL)
    → emits vehicle_detected / vehicle_idle events to events stream
    → dashboard renders with vehicle icon + inline snapshot thumbnail
```

### Design Decisions

- **Persistent tracking** — IoU matching tracks vehicles across frames with TrackedVehicle objects (center history, stationary detection for idle alerts)
- **Frame skip = 3** — processes every 3rd frame since vehicles move fast
- **Rate limiting** — max 1 `vehicle_detected` event per 3 seconds to prevent flood from steady traffic
...
- [x] 22 tool-calling functions: query events/events by date/event patterns/activity heatmap, query faces/unknowns/show face photos, live scene, capture snapshot (with weather + scene description), capture clip (5-second video), get weather, browse vehicles, query zones, query notification history, feedback stats, review feedback, retrain rules, send Telegram (text/snapshot/clip), schedule reminders, system status, record verdicts, analyze image (MiniCPM-V)
...
## Phase 9: QNAP NAS Storage — DVR Recording + Event Journal + Audit Trail  ✅ Complete

**Goal:** Persistent storage for all recordings, events, and audit data on the QNAP NAS.

### What Gets Built

| Component | Runs On | What It Does |
|-----------|---------|--------------|
| DVR recorder | PC (CPU) | ffmpeg remuxes RTSP sub-stream to 1-hour MPEG-TS segments on QNAP NAS |
| Event journal | Dashboard | Appends all events as JSON lines to daily files on QNAP NAS |
| Telegram audit trail | Dashboard | Logs every bot command, saves snapshot/clip copies per user on QNAP NAS |

### QNAP NAS Storage Layout

```
Q:\vision-labs\
├── recordings/     ← DVR MP4 segments (28-day retention, auto-cleanup)
│   └── front_door/
│       └── YYYY-MM-DD/
│           └── HH-MM.mp4   ← 1-hour segments, named by actual start time
├── snapshots/      ← Event snapshots with bbox highlights
├── events/         ← Daily JSONL event journals
│   └── YYYY-MM-DD.jsonl
├── telegram/       ← Per-user Telegram audit trail
│   └── @username/
│       ├── commands.jsonl
│       ├── snapshots/
│       └── clips/
├── vehicles/       ← Vehicle snapshots (future)
├── faces/          ← Unknown face crops (future)
├── clips/          ← Event clips (future)
└── feedback/       ← Feedback data (future)
```

### Docker CIFS Volumes

4 QNAP CIFS/SMB volumes mount directly into containers:
- `qnap-snapshots` → `//192.168.1.250/vision-labs/snapshots`
- `qnap-recordings` → `//192.168.1.250/vision-labs/recordings`
- `qnap-events` → `//192.168.1.250/vision-labs/events`
- `qnap-telegram` → `//192.168.1.250/vision-labs/telegram`

> [!NOTE]
> SQLite databases (faces, auth, feedback, AI) remain on local Docker volumes.
> SQLite is incompatible with SMB/CIFS due to file locking limitations.

### Done When

- [x] DVR recorder writing MP4 segments to QNAP NAS
- [x] 28-day automatic retention cleanup
- [x] Event journal logging all events as JSONL
- [x] Telegram audit trail logging commands + saving media
- [x] All 9 Docker services healthy
...
graph TD
    ...
    P7 --> P7_5["Phase 7.5: Telegram Access ✅"]
    P7_5 --> P8["Phase 8: Extended Bot + AI Tools ✅"]
    P8 --> P9["Phase 9: QNAP NAS Storage ✅"]
    P9 --> P10["Phase 10: Image Generation ✅"]
    P10 --> P11["Phase 11: Vision Analysis ✅"]
    P10 --> P12["Phase 12: Video Production ✅"]
    ...
    style P7_5 fill:#2d5016,color:#fff
    style P8 fill:#2d5016,color:#fff
    style P9 fill:#2d5016,color:#fff
    style P10 fill:#2d5016,color:#fff
    style P11 fill:#2d5016,color:#fff
    style P12 fill:#2d5016,color:#fff
```
...
| **Phase 6** | Zones + alerts + Telegram notifications | ✅ Complete |
| **Phase 6.2** | Vehicle detection (car/truck/bus/motorcycle) | ✅ Complete |
| **Phase 6.5** | Self-learning feedback loop | ✅ Complete |
| **Phase 7** | LLM brain (Ollama + Qwen 3 14B, 22 tools) | ✅ Complete |
| **Phase 7.5** | Telegram Access Manager | ✅ Complete |
| **Phase 8** | Extended bot commands + AI tools | ✅ Complete |
| **Phase 9** | QNAP NAS storage (DVR + event journal + audit trail) | ✅ Complete |
| **Phase 10** | Image generation (SDXL + LoRA + sweeps + gallery) | ✅ Complete |
| **Phase 11** | Vision analysis (MiniCPM-V multimodal) | ✅ Complete |
| **Phase 12** | Video production (AnimateDiff + TTS + characters) | ✅ Complete |
...
> Phases 4-6 make it exceptional. Phases 7+ are advanced capabilities.
